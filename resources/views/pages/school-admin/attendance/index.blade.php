...
@section('content')
<div
    x-data="schoolAdminAttendanceIndex({
        indexUrl:     '{{ route('school-admin.attendance.index') }}',
        semestersUrl: '{{ route('school-admin.attendance.semesters') }}',
...
>
    ...
    <div style="margin-bottom:16px">
        <ul class="breadcrumb-list">
            ...
            <li><span>Presensi Sekolah</span></li>
        </ul>
    </div>
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:20px;flex-wrap:wrap">
        <div>
            ...
            <p style="font-size:13px;color:var(--text-muted);margin-top:3px">
                Riwayat seluruh sesi absensi dari semua guru
            </p>
        </div>
    </div>
    ...
    <div class="dt-wrap">
        ...
        <div class="ta-table-wrap">
                    ...
                    <template x-if="!loading && sessions.length > 0">
                        <template x-for="(s, i) in sessions" :key="s.attendance_session_id">
                            <tr>
                                ...
                                <td>
                                    <div style="display:flex;align-items:center;gap:10px">
                                        <div class="dt-av av-purple" x-text="initials(s.teacher_name)"></div>
                                        <div>
                                            <div class="dt-user-name" x-text="s.teacher_name ?? '—'"></div>
                                        </div>
                                    </div>
                                </td>
                                <td>
            ...
            <div x-show="!loading && sessions.length === 0" class="ta-empty">
                ...
                <div class="ta-empty-sub">Sesi absensi akan muncul setelah guru melakukan presensi dari jadwal masing-masing</div>
            </div>
        </div>
...
@push('scripts')
<script>
function schoolAdminAttendanceIndex(config = {}) {
    return {
        indexUrl:     config.indexUrl     ?? '/school-admin/attendance',
        semestersUrl: config.semestersUrl ?? '/school-admin/attendance/semesters',
